...
import { Circle, Clock, CheckCircle2, Ban, ExternalLink, Bug, BugOff, Info } from "lucide-react"
...
import type { Component, UserComponentStatus, ComponentStatus } from "@/lib/types"
import type { ComponentStats } from "@/app/testing/page"
...
export function ComponentList({
  ...
  const [localStatuses, setLocalStatuses] = useState<Record<string, UserComponentStatus>>(() => {
  ...
  const handleStatusChange = async (componentId: string, status: ComponentStatus) => {
    const existing = getStatus(componentId)
    const oldStatus = existing?.status || "not_started"
      ...
      <Table>
        <TableHeader>
          <TableRow>
                ...
                <Dialog open={isStatusInfoOpen} onOpenChange={setIsStatusInfoOpen}>
                  ...
                  <DialogContent className="max-w-4xl max-h-[85vh] overflow-auto">
                    <DialogHeader>
                      ...
                      <DialogDescription>What each testing status means.</DialogDescription>
                    </DialogHeader>
                    <div className="space-y-3 text-sm">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-muted-foreground">Haven&apos;t begun testing</span>
                      </div>
                      <div className="flex items-center gap-2">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-muted-foreground">Finished testing</span>
                      </div>
                      <div className="flex items-center gap-2">
            ...
            <TableHead className="w-16 text-right border-l">Action</TableHead>
          </TableRow>
          <TableRow className="hover:bg-transparent">
            ...
            <TableHead className="text-center w-16 py-1">
              ...
              <TableHead className="text-center border-l w-20 py-1 text-xs text-muted-foreground">Total</TableHead>
            )}
            <TableHead className="text-center border-l w-16 py-1">
              <Tooltip>
                <TooltipTrigger asChild>
        ...
        <TableBody>
          {components.map((component) => {
            ...
            const status = (userStatus?.status ?? "not_started") as ComponentStatus
            ...
            const compBugCounts = stats.bugCounts[component.id] || { open: 0, closed: 0 }
            const totalTesters =
            ...
            return (
              <TableRow key={component.id} className={cn(isSelected && "bg-primary/5", !isSelected && "opacity-60")}>
                <TableCell>
                  <Checkbox
                    checked={isSelected}
                    onCheckedChange={(checked) => handleSelectToggle(component.id, checked === true)}
                  />
                </TableCell>
                <TableCell>
                  <div>
                ...
                <TableCell>
                  <Select
                    value={status}
                    onValueChange={(value) => handleStatusChange(component.id, value as ComponentStatus)}
                    disabled={!isSelected}
                  >
                    <SelectTrigger
                      className={cn("h-8 text-xs w-[140px]", config.selectClassName)}
                      disabled={!isSelected}
                    >
                      <SelectValue />
                    </SelectTrigger>
                ...
                <TableCell className="text-center tabular-nums text-sm text-red-600">
                  ...
                  <TableCell className="text-center border-l tabular-nums text-sm">
                    {totalTesters > 0 ? (
                      <span className="text-muted-foreground">{totalTesters}</span>
                    ) : (
                      <span className="text-xs font-semibold text-red-600">Needs testers!</span>
                    )}
                  </TableCell>
                )}
                <TableCell className="text-center border-l tabular-nums text-sm">
                  {compBugCounts.open > 0 ? (
                    <span className="text-orange-600 font-medium">{compBugCounts.open}</span>
                  ) : (
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Link
                        ...
                        className={cn(!isSelected && "pointer-events-none opacity-50")}
                      >
