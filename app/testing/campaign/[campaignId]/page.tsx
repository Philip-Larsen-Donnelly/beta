...
export default async function CampaignTestingPage({ params }: { params: Promise<{ campaignId: string }> }) {
  ...
  const categoryMappings = await fetchCategoryMappingsForComponents(componentIds)
  ...
  const { rows: allBugs } = componentIds.length
    ? await query("SELECT component_id, status FROM bugs WHERE component_id = ANY($1::uuid[])", [componentIds])
    : { rows: [] }

  const statusCounts: Record<string, Record<string, number>> = {}
  allStatuses?.forEach((s) => {
    if (!statusCounts[s.component_id]) {
      statusCounts[s.component_id] = { not_started: 0, in_progress: 0, completed: 0, blocked: 0 }
    }
    statusCounts[s.component_id][s.status] = (statusCounts[s.component_id][s.status] || 0) + 1
  })
  ...
  const stats: ComponentStats = { statusCounts, bugCounts }
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-sm text-muted-foreground">
        Please select only the components you intend to test, so that we can get an accurate idea of how much testing
